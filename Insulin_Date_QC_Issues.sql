...
--Please see the attached spreadsheet for the list of mice with incorrect dates.
...
--Change those incorrect insulin blood collection dates to the correct necropsies dates (I have checked that most necropsies dates are the same as Mosaic death dates).
...
WHERE necropsies.datetime = insulins.datetime
 ...
 Change those incorrect insulin datetimes to the correct blood collection dates (I have checked that necropsies dates are the same as Mosaic death dates).
...
WHERE insulins.blood_collection_datetime > insulins.datetime
 ...
 Fix the mismatched Mosaic death dates, necropsies dates, insulin datetimes, insulin blood collection dates which are after insulin datetimes (see the results from the two SQL queries below).
...
--Mosaic death dates, insulin datetimes, insulin blood collection dates are mismatched.

--Status code those incorrect blood collection dates as shown in the results from the SQL queries below
