...
--1. Blood collection/sacrifice date is more than 14 days earlier or later than timepoint specified by impress:
...
--2. Fix blood collection/sacrifice dates that occur after date of experiment
...
--3. Update necropsies to match hemas.datetime for consistent timestamps
UPDATE hemas
...
--4. Set incorrect datetimes to correct blood collection/sacrifice dates where collection_datetime matches Mosaic
UPDATE hemas
JOIN mbp.labtracks_animal_mirror ltam ON hemas.mouse_name = CONCAT(stock, '-', pedigree_number)
...
--5. Summary of cases where mosaic death_date !=  hemas.datetime
